<?php
  ...
  //grab sort selection and keyword search from landing page search
  $sidebarSelection = $_GET["searchID"];
  $sortSelection = $_GET["sortBy"];
  $searchSelection = $_GET["searchResults"];

  if (isset($_POST["submit"])) {

      $searchResults = $_POST["searchResults"];
      $dropdownSelection = $_POST["sortDropdown"];
      $carriedSidebarSelection = $_POST["sidebarSelection"];

      if ($searchResults !== "") {
        header("Location: seniorProjectDiscardedCategorySidebarAndFixedSearch.php.php?searchID=".$carriedSidebarSelection."&sortBy=".$dropdownSelection."&searchResults=".$searchResults);
      }
      else {
        header("Location: seniorProjectDiscardedCategorySidebarAndFixedSearch.php?searchID=".$carriedSidebarSelection."&sortBy=".$dropdownSelection);
      }
  ...
  <nav class="navbar navbar-inverse navbar-fixed-top" style="background-color: #00336f">
    ...
    <div id="wrapper">
        ...
        <div id="page-content-wrapper">
            <div class="container-fluid">
                <?php
                echo '<h1 style="font-size: 75px" align="center">'.$sidebarSelection.' Discarded</h1>'
                ?>
                &nbsp;
              <form action='seniorProjectDiscardedCategorySidebarAndFixedSearch.php' method='POST' class='form-group'>
                <div class="form-horizontal" align="center">
                  <input type="text" name="searchResults" 
                    class="form-control" 
                    <?php
                    echo "placeholder='".$searchSelection."'";
                    ?>
                ...
                <div class="row" align="center">
                  <div class="form-group" style="width: 50%" align="center">
                    <select id="dropdownForm" name="sortDropdown" class="form-control">
                      ...
                      <?php
                      if ($sortSelection == "LocationID") {
                        echo "selected=selected";
                      }
                      ?>>Location</option>;
                      <option value="CategoryID"
                      <?php
                      if ($sortSelection == "CategoryID") {
                        echo "selected=selected";
                      }
                      ?>>Category</option>;
                      <option value="Quantity"
                      <?php
                      if ($sortSelection == "Quantity") {
                        echo "selected=selected";
                      }
                      ?>>Quantity</option>;
                      <option value="LastQuantityUpdate"
                      <?php
                      if ($sortSelection == "LastQuantityUpdate") {
                        echo "selected=selected";
                      }
                ...
                <input type="hidden" name="sidebarSelection" value="<?php echo $sidebarSelection; ?>">
              </form>
                &nbsp;&nbsp;&nbsp;&nbsp;
                <div class="container">
                      ...
                      <tbody>
                        <tr>
                          <?php
                              ...
                              $inventoryQuery .= "FROM Discarded NATURAL JOIN User NATURAL JOIN Location NATURAL JOIN Category WHERE Name LIKE '%".$searchSelection."%' AND CategoryName LIKE '%".$sidebarSelection."%' OR LocationName LIKE '%".$searchSelection."%' AND CategoryName LIKE '%".$sidebarSelection."%' OR QuantityDiscarded LIKE '%".$searchSelection."%' AND CategoryName LIKE '%".$sidebarSelection."%' OR LastDiscardUpdate LIKE '%".$searchSelection."%' AND CategoryName LIKE '%".$sidebarSelection."%' OR FirstName LIKE '%".$searchSelection."%' AND CategoryName LIKE '%".$sidebarSelection."%' OR LastName LIKE '%".$searchSelection."%' AND CategoryName LIKE '%".$sidebarSelection."%' OR DescriptionDiscarded LIKE '%".$searchSelection."%' AND CategoryName LIKE '%".$sidebarSelection."%' ORDER BY ".$sortSelection."";
                                    ...
                                    echo "<tr>";
                                        ...
                                        echo "<td><center><div class='dropdown'>";
                                          ...
                                          echo "<ul class='dropdown-menu'>";
                                            echo "<li><a href='seniorProjectDiscardedDeleteItem.php?itemID=".$row['InventoryNum']."'>Delete Item</a></li>";
                                          echo "</ul>";
                                        echo "</div></center></td>";
                                    echo "</tr>";
